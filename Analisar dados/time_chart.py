...
def chart_generate(file_path):
    ...
    grouped_bar = (data.groupby(["Nivel", "Lingua"]) [["Tempo"]].sum().reset_index())
    ...
    chart.set_subtitle("Tempo de Compressão - Media")
    chart.axes.set_yaxis_tick_orientation("horizontal")
    chart.axes.set_xaxis_label("Tempo (segundos)")
